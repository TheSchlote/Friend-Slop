...
CI enforces basic architecture drift through `ArchitectureGuardrailTests`: no new singleton-style globals, no direct scene searches in frame methods, no growing already-oversized runtime files, and no wrong-way runtime/UI/editor asmdef references. The singleton policy and migration history are documented in [architecture.md](architecture.md) §D-014.
